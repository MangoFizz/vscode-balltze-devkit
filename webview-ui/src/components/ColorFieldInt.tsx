...
const ColorFieldInt: React.FC<ColorArgbFieldProps> = ({ label, value, setValue }) => {
	...
	let handleColorPicker = (e: any) => {
		let getChannel = (i: number): number => {
			return parseInt(e.target.value.substr(i, 2), 16);
		};
	...
	let getColorHexValue = () => {
		let colorHex = "#";
		let getColorHex = (channel: number): string => channel.toString(16).padStart(2, '0');
